...
**Study-mode separation (2026-07-02).** The live participant deployment runs with `APP_MODE=study` (frontend env) + `VC_APP_MODE=study` + `VC_RESEARCH_KEY` (backend env): `frontend/middleware.ts` redirects `/` → `/study/survey` and 404s `/simulate`·`/research`·`/pscon`; the backend **does not mount** the `simulations`/`synthesis`/`pscon` routers (simulation data structurally cannot enter the study DB) and gates `research`+`exports` behind the key (`X-Research-Key` header or `?key=`; **fail-closed** when the key is unset in study mode — `app/core/research_gate.py`). Local dev sets none of these envs → everything open as before. Researcher access to live data keeps working with the key: `scripts/download_study_sessions.py` reads it from `VC_RESEARCH_KEY` env or `frontend/.env.local` (`NEXT_PUBLIC_RESEARCH_KEY` — a local frontend pointed at the live backend attaches it on every request via `lib/api.ts`); the key itself lives in Railway backend variables (`railway variables --service valuecommit-backend`), never in the repo.
