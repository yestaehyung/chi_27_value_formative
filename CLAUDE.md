...
- `app/agents/service_agent.py` — turn/feedback orchestration; `action_selector.py` (structural guards + LLM `action_decision`), `response_generator.py` (LLM rewrites Korean templates; strips markdown). **Next-action flow (post 2026-06-25 recommend-first pivot):** `select_next_action` resolves only structural guards — `show_conflict`/`close`/`explain` (keyed on real conflicts/dialogue-acts/impression existence) — and otherwise returns the `llm_decide` sentinel; the LLM `action_decision` task then decides **recommend vs clarify** and, when clarify, authors the probe dimension (constrained to the 12-vocab) + the question text. `question_strategy.build_value_question` is now only the fallback used when the LLM clarifies without supplying a question. (The earlier *3-tier clarify ladder* — motivation probe → RIG anticipatory → value-level clarify — was retired in the pivot; `next_probe`/`should_value_clarify` were **deleted** 2026-06-27. Motivation is **detect-only**, never conversationally asked; RIG enters the decision only as the `ragPrediction` context field, not a separate tier.) On every recommend turn the reply is generated **after** `rerank_by_intent` + `select_tradeoff_set`, grounded on the actual shown set (not the pre-rerank pool).
