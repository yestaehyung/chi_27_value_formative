...
export default function ConflictCard({
  ...
  onDismiss?: () => void;
  /** 답변 생성 중 해소 잠금 — 해소가 항상 다음 추천에 반영되도록(레이스·구상태 응답 방지). */
  disabled?: boolean;
}) {
    ...
    <div className="msg-in card overflow-hidden border-[#ecc94b]">
      <div className="flex items-start justify-between bg-[#fffbe6] px-4 py-3">
        <h3 className="flex items-center gap-2 text-sm font-bold text-[#191919]">
          ...
          {tr("기준이 바뀐 것 같아요", "Your Criteria May Have Changed")}
        </h3>
        {onDismiss && (
          <button onClick={onDismiss} className="text-xs text-[#8a6d00] hover:underline">{tr("나중에", "Later")}</button>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-[11px]">
          <div className="rounded-xl bg-[#f5f6f8] p-2.5">
            <div className="font-semibold text-[#9aa0a6]">{tr("지금까지 이해", "Previous understanding")}</div>
            <div className="mt-0.5 text-[#404040]">{conflict.oldAssumption}</div>
          </div>
          <div className="rounded-xl bg-[#ecfdf5] p-2.5">
            <div className="font-semibold text-[#4f46e5]">{tr("방금 보인 것", "Recent signal")}</div>
        ...
        {!manualMode ? (
          <div className="mt-3 space-y-1.5">
            ...
            {options.some((o) => o.action === "manual_edit") && (
              ...
              >
                {tr("직접 수정하기", "Edit Manually")}
              </button>
            )}
            {disabled && (
              <div className="rounded-xl bg-[#f5f6f8] px-3 py-2 text-[11px] text-[#9aa0a6]">
                {tr("답변을 만드는 중이에요 — 잠시 후 선택할 수 있어요.", "The agent is preparing a response. You can choose shortly.")}
              </div>
            )}
            {hoveredOption && (
          ...
          <div className="mt-3">
            <textarea
              ...
              rows={2}
              placeholder={tr("원하는 기준을 직접 적어주세요", "Enter the criterion you want")}
              className="chat-input w-full resize-none px-3.5 py-2.5 text-xs focus:outline-none"
            />
            <div className="mt-2 flex justify-end gap-1.5">
              <button className="btn px-3 py-1.5 text-xs" onClick={() => setManualMode(false)}>{tr("취소", "Cancel")}</button>
              <button
                className="btn btn-primary px-3 py-1.5 text-xs"
                disabled={disabled || !manualText.trim()}
                onClick={() => onResolve("manual_edit", manualText.trim())}
              >
                {tr("이 기준으로 수정", "Use This Criterion")}
              </button>
            </div>
          </div>
        )}
