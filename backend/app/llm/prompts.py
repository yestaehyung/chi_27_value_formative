...
판단 원칙:
1. **제약이 1순위다.** statedConstraintsNote의 예산·필수·비선호를 어기는 후보는 아래로
   내린다. 예산은 후보의 price로 직접 확인하고, 비선호("커널형 싫음" 등)는 후보의
   keyAttributes·productType·제목(없으면 설명)에서 해당 특성을 읽어 판단한다.
2. **상품 종류·대상 정합.** 대화가 찾는 상품 종류(productType)와 다르거나, 대상(audience)이
   맞지 않는 후보(성인 쇼핑에 아동용 등)는 맨 아래로.
...
6. **상위 5개는 실제로 노출되는 셋이다 — 보완적으로 구성하라.** 제약을 지키는 후보들
   중에서, 강점이 서로 다른 5개(예: 가성비형 / 검증·신뢰형 / 특색형 / 프리미엄형 /
   실용 기본형)를 상위에 올려 사용자가 의미 있는 선택을 할 수 있게 한다. 거의 같은
   상품이 나란히 오는 구성은 피하고, 서로 다른 기준을 대표하는 조합을 만든다.
...
recommend일 때 인자 2개를 함께 만든다:
- searchText: 이 대화의 사용자 발화 전체(userUtterances)와 피드백(feedbackEvents)을 반영한, 그 자체로 완결된 한국어 상품 검색문.
  찾는 상품 종류 + 원하는 특징(긍정 신호)만 담는다. 예산·비선호·"~빼고" 같은 제약은 searchText가 아니라 constraintsNote에 쓴다
  (임베딩 검색은 부정을 읽지 못한다 — "커널형 싫음"을 넣으면 커널형이 더 잘 검색된다).
- constraintsNote: 예산 상한·필수 조건·비선호를 자연어 한두 문장으로 요약. 없으면 빈 문자열.
