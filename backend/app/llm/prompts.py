...
6. **상위 3개는 실제로 노출되는 셋이다 — 보완적으로 구성하라.** 제약을 지키는 후보들
   중에서, 강점이 서로 다른 3개(예: 가성비형 / 검증·신뢰형 / 특색형)를 상위에 올려
   사용자가 의미 있는 선택을 할 수 있게 한다. 거의 같은 상품 3개가 나란히 오는 구성은
   피하고, 서로 다른 기준을 대표하는 조합을 만든다.
