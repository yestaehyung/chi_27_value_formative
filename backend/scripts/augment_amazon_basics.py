"""seed_amazon 기본 의류 증보 — 일상 옷 8종 (2026-07-03, "일반적인 옷들도 추가" 요청).
현재 의류가 원피스·코트·수영복뿐이라 일상 의류(티셔츠·바지·니트 등)가 공백 — 자유
대화("출근용 셔츠 추천해줘")가 성립하도록 기본 카테고리를 채운다. 시나리오는 추후.

augment_amazon_summer.py와 같은 계열(셋째) — Clothing 메타를 스트리밍하며 카테고리
경로 노드로 정밀 필터(제목 정규식 아님), 가격+이미지+리뷰 30+만 수집, 신규만 번역·축약.

파이프라인 정합(중요):
- 레코드 형태는 기존과 동일: title(간결 카드명) / attributes.titleEn / attributes.titleFull /
  USD→KRW 100원 반올림 / category ∈ 8종 기본 의류 라벨.
- 이후 단계: build_product_profiles.py(증분) → 로컬 임베딩 갱신 → 커밋 → VC_SEED_UPSERT=1 1회.
  ...
  cd backend && VC_LLM_PROVIDER=deepseek VC_DEEPSEEK_MODEL=deepseek-v4-flash VC_DEEPSEEK_THINKING=off \
      ...
      .venv/bin/python scripts/augment_amazon_basics.py
"""
import asyncio
import json
import os
...
# 남녀 노드 변형을 합친다 (없는 노드명은 그냥 안 걸릴 뿐 — 안전).
KINDS: dict[str, dict] = {
    "티셔츠": {"nodes": {"T-Shirts"}},
...
async def main():
    ...
    print(f"기존 풀: {len(products)}개 — 기본 의류 8종 증보 시작 (각 {N_EACH})")
