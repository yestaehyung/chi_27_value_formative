"""seed_amazon 여름 상품 증보 — 샌들·수영복·선글라스 (2026-07-03, 스터디가 7월 진행이라
계절 정합 상품군 추가; 시나리오는 나중에 — 풀만 넓혀 자유 대화에서 쓰이게 한다).

augment_amazon_womens_outerwear.py와 같은 계열 — Clothing 메타를 스트리밍하며 카테고리
경로 노드로 정밀 필터(제목 정규식 아님), 가격+이미지+리뷰 30+만 수집, 신규만 번역·축약.

파이프라인 정합(중요):
- 레코드 형태는 기존과 동일: title(간결 카드명) / attributes.titleEn / attributes.titleFull /
  USD→KRW 100원 반올림 / category ∈ {샌들, 수영복, 선글라스}.
- 이후 단계: build_product_profiles.py(증분) → 로컬 임베딩 갱신 → 커밋 → VC_SEED_UPSERT=1 1회.
  ...
  cd backend && VC_LLM_PROVIDER=deepseek VC_DEEPSEEK_MODEL=deepseek-v4-flash VC_DEEPSEEK_THINKING=off \
      N_EACH=60 LINE_CAP=2000000 PYTHONPATH=. \
      .venv/bin/python scripts/augment_amazon_summer.py
"""
import asyncio
import json
import os
...
LINE_CAP = int(os.environ.get("LINE_CAP", "2000000"))
...
# 카테고리 경로 노드 매칭 (정밀) — 제목 정규식은 오분류를 만든다 (build_pool.py 교훈).
# 수영복은 여성(Swimsuits & Cover Ups)·남성(Swim Trunks/Board Shorts) 노드를 합친다.
KINDS: dict[str, dict] = {
    "샌들": {"nodes": {"Sandals"}},
...
async def main():
    ...
    print(f"기존 풀: {len(products)}개 — 여름 상품 증보 시작 (샌들/수영복/선글라스 각 {N_EACH})")
