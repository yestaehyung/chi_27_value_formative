"""seed_amazon 부담 없는 선물 5종 신설 — 양말·모자·스카프·텀블러머그·캔들 각 300 (2026-07-04).
"너무 비싼 선물 말고 주변에 할 만한 것" — per-kind 가격 상한(max_usd)을 둔 캐주얼 선물 티어.

참가자들이 선물 목적 쇼핑 경향을 보여(사용자 관찰) 선물 대표 카테고리를 추가한다.
expand300과 병렬 실행을 위해 products.json이 아닌 **스테이징 파일**에 쓴다
(products_casualgifts_staging.json) — 두 작업 완료 후 병합(id dedupe).



  cd backend && VC_LLM_PROVIDER=deepseek VC_DEEPSEEK_MODEL=deepseek-v4-flash VC_DEEPSEEK_THINKING=off \
      PYTHONPATH=. .venv/bin/python scripts/augment_amazon_casual_gifts.py
"""
import asyncio
import json
import os
...
PLAN: dict[str, dict[str, dict]] = {
    "meta_Clothing_Shoes_and_Jewelry": {
        "양말": {"nodes": {"Socks", "Casual Socks", "Athletic Socks", "Dress Socks", "Socks & Hosiery"}, "goal": 300, "men_min": 90, "max_usd": 40},
        "모자·캡": {"nodes": {"Baseball Caps", "Beanies & Knit Hats", "Sun Hats", "Bucket Hats", "Hats & Caps"}, "goal": 300, "men_min": 90, "max_usd": 40},
        "스카프·머플러": {"nodes": {"Scarves", "Fashion Scarves", "Cold Weather Scarves & Wraps", "Scarves & Wraps"}, "goal": 300, "men_min": 60, "max_usd": 40},
    },
    "meta_Home_and_Kitchen": {
        "텀블러·머그": {"nodes": {"Mugs", "Travel Mugs", "Tumblers & Water Glasses", "Insulated Tumblers"}, "goal": 300, "men_min": 0, "max_usd": 40},
        "캔들·디퓨저": {"nodes": {"Candles", "Scented Candles", "Jar Candles", "Essential Oil Diffusers", "Aromatherapy Diffusers"}, "goal": 300, "men_min": 0, "max_usd": 40},
    },
}
...
async def main():
    ...
    print(f"기존 풀: {len(products)}개 — 캐주얼 선물 5종 수집 시작 (스테이징)")
    ...
    new_items += collect_file("meta_Home_and_Kitchen",
                              PLAN["meta_Home_and_Kitchen"], CLOTH_JUNK,
                              CLOTHING_CAP, existing_ids)
    if not new_items:
    ...
    (OUT / "products_casualgifts_staging.json").write_text(
        json.dumps(new_items, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"\n완료 — 스테이징 {len(new_items)}개 → products_casualgifts_staging.json (병합은 별도 단계)")
