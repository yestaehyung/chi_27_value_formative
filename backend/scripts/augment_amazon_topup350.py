...
PLAN: dict[str, dict[str, dict]] = {
    "meta_Clothing_Shoes_and_Jewelry": {
        "티셔츠": {"nodes": {"T-Shirts"}, "goal": 14, "men_min": 7},
        "시계": {"nodes": {"Wrist Watches"}, "goal": 8, "men_min": 4},
        "모자·캡": {"nodes": {"Baseball Caps", "Beanies & Knit Hats", "Sun Hats", "Bucket Hats", "Hats & Caps"}, "goal": 4, "men_min": 2, "max_usd": 40},
    },
    "meta_Electronics": {
        "무선이어폰": {"nodes": {"Earbud Headphones", "In-Ear Headphones"}, "goal": 4, "men_min": 0},
        "블루투스 스피커": {"nodes": {"Portable Bluetooth Speakers", "Bluetooth Speakers"}, "goal": 4, "men_min": 0},
    },
}
...
async def main():
    ...
    new_items += collect_file("meta_Electronics", PLAN["meta_Electronics"], ELEC_JUNK2,
                              CLOTHING_CAP, existing_ids)
    if not new_items:
