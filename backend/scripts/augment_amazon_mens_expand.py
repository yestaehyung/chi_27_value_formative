"""남성 의류 대확장 — 성별 균형 목표(일반 45%/유니섹스 30%) 기반 증보 (2026-07-06).
'나는 남자야' 무시 사례 재발 방지의 데이터 축: 전원 남성("Men" 경로 필수) 수집 +
번역 카드명에 '남성' 표기 강제(임베딩 성별 신호). 목표량은 실측 격차에서 산출."""
import asyncio
import json
import os
import sys
...
PLAN: dict[str, dict[str, dict]] = {
    "meta_Clothing_Shoes_and_Jewelry": {
        "티셔츠": {"nodes": {"T-Shirts"}, "goal": 67, "men_min": 67},
        "니트·스웨터": {"nodes": {"Sweaters", "Pullovers", "Cardigans"}, "goal": 65, "men_min": 65},
        "후드·맨투맨": {"nodes": {"Fashion Hoodies & Sweatshirts", "Sweatshirts & Hoodies", "Hoodies & Sweatshirts"}, "goal": 72, "men_min": 72},
        "샌들": {"nodes": {"Sandals"}, "goal": 57, "men_min": 57},
        "수영복": {"nodes": {"Swim Trunks", "Board Shorts", "Rash Guards", "Swim"}, "goal": 113, "men_min": 113},
        "코트": {"nodes": {"Coats, Jackets & Vests", "Jackets & Coats"}, "goal": 56, "men_min": 56},
        "양말": {"nodes": {"Socks", "Casual Socks", "Athletic Socks", "Dress Socks", "Socks & Hosiery"}, "max_usd": 40, "goal": 74, "men_min": 74},
        "모자·캡": {"nodes": {"Baseball Caps", "Beanies & Knit Hats", "Sun Hats", "Bucket Hats", "Hats & Caps"}, "max_usd": 40, "goal": 82, "men_min": 82},
        "선글라스": {"nodes": {"Sunglasses"}, "goal": 78, "men_min": 78},
    },
}
...
async def koreanize_one(provider, sem, p, counter, total):
    msg = ("다음 아마존 상품을 한국 쇼핑앱 스타일로 가공해. JSON만 출력:\n"
           ...
           ' 규칙: 남성용 상품이므로 카드명 맨앞을 브랜드로 두되 반드시 남성 단어를 포함해. 무릎길이 데님 반바지면 버뮤다도 포함.\n'
